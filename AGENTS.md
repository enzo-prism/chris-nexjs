...
`drizzle.config.ts` requires `DATABASE_URL`; load it through a local, untracked `.env`. Avoid logging patient identifiers or submission payloads. Keep canonical/redirect behavior synchronized between `shared/redirects.ts`, middleware behavior, and `vercel.json`. Keep doctor name formatting compliant in content updates: use either `Dr. Christopher B. Wong` or `Christopher B. Wong, DDS` (never both in one line). Payment-method copy must match accepted methods: Visa, MasterCard, FSA/HSA, and the in-house dental plan (no CareCredit unless product changes); keep Insurance UI and the `data.ts` payment FAQ in lockstep. Avoid keyword-stuffed body openers and do not put SEO phrases into Person **or** WebSite `alternateName`. Person schema `name` is `Christopher B. Wong` with honorificPrefix/Suffix — do not duplicate Dr. in the name string. `/thank-you` meta must not claim a scheduled appointment. After substantive copy changes, update `LASTMOD_OVERRIDES` in `shared/seo.ts` for touched routes. Homepage review quotes live in `client/src/data/homeProof.ts` and must stay verbatim excerpts of the Google export (`pnpm test:reviews` enforces it). Lead attribution (`shared/attribution.ts`, `client/src/lib/attribution.ts`) may carry only a channel, landing path, referrer host, and `utm_*` tags. Never add identifiers.
