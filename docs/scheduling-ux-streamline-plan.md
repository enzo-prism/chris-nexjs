...
Canonical frontend payload is `ScheduleRequestV2` in [scheduleRequest.ts](../shared/scheduleRequest.ts):
...
Legacy payloads are still accepted by [route.ts](../app/api/schedule-request/route.ts) and normalized into v2.
